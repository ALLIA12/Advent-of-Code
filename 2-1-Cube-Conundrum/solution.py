...
def getOneRowVal(str, reds, greens, blues):
    ...
    for color_string in color_counts_segments:
        ...
        for match in matches:
            if match[1] == 'red':
                ...
                if int(match[0]) > greens:
                    return 0
            elif match[1] == 'blue':
                if int(match[0]) > blues:
                    return 0
    return int(game_id)


def getSum(string_array, reds, greens, blues):
    sum = 0
    for str in string_array:
        sum += getOneRowVal(str, reds, greens, blues)
    return sum
...
if __name__ == "__main__":
    ...
    result = getSum(string_array, reds, greens, blues)
    print(result)
